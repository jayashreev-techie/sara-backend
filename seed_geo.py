"""
Seed script for countries, states, and districts master data.
Run: python seed_geo.py
"""
from app.database import engine, MasterBase, SessionLocal
...
def seed():
    ...
    try:
        ...
        for state_name, districts in INDIA_STATES_DISTRICTS.items():
            state = State(state_name=state_name, country_id=india.id)
            db.add(state)
            db.flush()
            total_states += 1

            for district_name in districts:
                db.add(District(district_name=district_name, state_id=state.id))
                total_districts += 1

        db.commit()
        print(f"Seeded: 1 country, {total_states} states/UTs, {total_districts} districts")
